...
import com.example.habittrackerapp.databinding.ActivityOnboardingBinding
    ...
    private fun actualizarDots(paginaActiva: Int) {
        dots.forEachIndexed { index, dot ->
            dot.setImageResource(
                if (index == paginaActiva) R.drawable.bg_day_completed
                else R.drawable.bg_day_today
            )
        }
    }
    // Cambiamos el texto del botón en la última página
    private fun actualizarBoton(posicion: Int, total: Int) {
        binding.btnNext.text = getString(
            if (posicion == total - 1) R.string.onboarding_start
